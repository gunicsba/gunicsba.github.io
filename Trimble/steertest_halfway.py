#!/usr/bin/env python3
"""
SAM200 motor bench-drive over PCAN: slow move to halfway, then fast return
to center. Same handshake/engage/disengage sequence as steertest.py, just a
...
HALFWAY      = 200      # target position for the slow outbound move (counts)
SLOW_CPS     = 200      # outbound ramp rate, counts per second (validated earlier)
FAST_CPS     = 800      # return ramp rate, counts per second
DWELL_S      = 0.3      # pause at halfway before returning
...
def main():
    ...
    # 5. slow move to HALFWAY, dwell, fast return to 0 -- then stop (one shot)
    slow_step = SLOW_CPS * TICK_DT
    fast_step = FAST_CPS * TICK_DT
    t0 = time.time()
    tick = 0
    target = 0.0
    phase = "out"
    dwell_start = None
    print(f"engaged. profile: 0 -> {HALFWAY} @ {SLOW_CPS}cps, "
          f"dwell {DWELL_S}s, -> 0 @ {FAST_CPS}cps. Ctrl-C to abort.")
    try:
        while not stop.is_set():
            if phase == "out":
                ...
                if time.time() - dwell_start >= DWELL_S:
                    phase = "back"
            elif phase == "back":
                target -= fast_step
                if target <= 0:
