#!/usr/bin/env python3
"""
SAM200 motor bench-drive over PCAN: turn right to TARGET, hold, then return
to center over BACK_S seconds. Same handshake/engage/disengage sequence as
steertest.py, just a different motion profile.

RUN THIS WITH THE MOTOR ALONE ON THE BUS (real receiver unplugged) and with the
output mechanically decoupled or wheels off the ground.
...
TARGET       = 5000     # target position, right turn (counts, positive = right)
OUT_S        = 2.0      # time to reach TARGET (not specified as "slow" -> paced ramp)
HOLD_S       = 2.5      # hold at TARGET, 2-3s requested
BACK_S       = 2.0      # slow return to 0, requested duration
...
def main():
    ...
    # 0. settle time -- give the motor's state machine a moment before we
    #    start talking, in case a prior run just disengaged it
    time.sleep(1.5)
    ...
    # 3b. wait for a real position reading -- the CFX always has one before
    #     it engages (see trimble drive straight.trc @ msg 2665/2674/2675)
    wait_t0 = time.time()
    while state["pos"] is None and time.time() - wait_t0 < 2.0:
        heartbeat(bus, hb); hb += 1
        time.sleep(0.05)
    print(f"pos before engage: {state['pos']}")

    # 4. engage: echo the actual position back as a hold command (this is
    #    what the real CFX does immediately before zeroing/engaging -- our
    #    earlier runs skipped this and only sometimes engaged), then zero
    #    the counter to enter position control
    send(bus, [0xA1, 0x01, 0x00, 0x00] + le32(state["pos"] or 0))
    time.sleep(0.02)
    send(bus, [0xA1, 0x3D, 0x00, 0x00, 0, 0, 0, 0])
    time.sleep(0.05)

    # 5. out to TARGET over OUT_S, hold HOLD_S, back to 0 over BACK_S -- then stop
    out_step  = (TARGET / OUT_S)  * TICK_DT
    back_step = (TARGET / BACK_S) * TICK_DT
    t0 = time.time()
    tick = 0
    target = 0.0
    phase = "out"
    hold_start = None
    print(f"engaged. profile: 0 -> {TARGET} over {OUT_S}s, hold {HOLD_S}s, "
          f"-> 0 over {BACK_S}s. Ctrl-C to abort.")
    try:
        while not stop.is_set():
            if phase == "out":
                ...
                if time.time() - hold_start >= HOLD_S:
                    phase = "back"
            elif phase == "back":
                target -= back_step
                if target <= 0:
                ...
                print(f"t={time.time()-t0:5.1f}  reg00={state['reg00']}  "
                      f"target={target:7.0f}  pos={state['pos']}  -- move complete")
                break
            ...
            if tick % 100 == 0:            # 2 s heartbeat
                heartbeat(bus, hb); hb += 1
            if tick % 10 == 0:
                print(f"t={time.time()-t0:5.1f}  phase={phase:5s}  "
                      f"reg00={state['reg00']}  target={target:7.0f}  pos={state['pos']}")
